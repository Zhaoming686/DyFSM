...
public class AlgoGSPAN {
    ...
    private List<DFSCode> maxFSMtoRemove;

    public AlgoGSPAN() throws Exception {
    ...
    private void keepFSM() throws IOException, ClassNotFoundException {
        //最终的maxFSM
        for (DFSCode c : this.maxFSMtoRemove) {
...
//                    System.out.println(v1 + " " + v2 + " " + vMap.get(v1).id + " " + vMap.get(v2).id);
    ...
    private void SolveFrequentChangedEdge(Map<String, List<Integer>[]> EdgeSupportIds) {
        if (DataGlobal.FrequentEdgeLast.isEmpty()) {
            Iterator<Map.Entry<String, List<Integer>[]>> Edge_Infs = EdgeSupportIds.entrySet().iterator();
            while (Edge_Infs.hasNext()) {
                Map.Entry<String, List<Integer>[]> Edge_Inf = Edge_Infs.next();
        ...
        for (Entry<ExtendedEdge, Set<Integer>> entry : extensions.entrySet()) {
            ...
            if (isCanonical(newC)) {
                ...
                if (sup >= Settings.minSup) {
                    FreDesCount++;
                    this.frequentSubgraphs.put(newC, newGraphIDs);
...
//                    this.frequentCount++;
                    ...
                    DataGlobal.InfrequentSubgraphs.put(newC, newGraphIDs);
                }
            }
        }
...
//                this.maxFSMtoAdd.add(c);
    ...
    private void gSpanInc(DFSCode c, Set<Integer> graphIdsDB) throws Exception {
        ...
        //从旧边界中寻找增长点
        //the fringe to extend
        Map<DFSCode, Set<Integer>> SubgraphsToExtend = new HashMap<>();

        //the infrequent may be changed to frequent, so we need to record and remove them
        List<DFSCode> InfreToRemove = new ArrayList<>();

        //the newcs which need to test, if it's infrequent then we remove it.
        List<DFSCode> CanMaxToRemove = new ArrayList<>();

        gSpanDFSInc(new DFSCode(), graphIdsInc, SubgraphsToExtend, InfreToRemove, CanMaxToRemove);
        for (DFSCode code : InfreToRemove) {
            DataGlobal.InfrequentSubgraphs.remove(code);
        }

        //对增长点进行增长
        for (DFSCode Toextend : SubgraphsToExtend.keySet()) {
            this.frequentSubgraphs.put(Toextend, SubgraphsToExtend.get(Toextend));
//            this.frequentCount++;
            gSpanDFS(Toextend, SubgraphsToExtend.get(Toextend), CanMaxToRemove);
        }

        //输入更新的极大信息，用于最后更新极大
        for (DFSCode newC : CanMaxToRemove) {
            DFSCode c_parent = newC.copy();
            c_parent.remove();
            if (DataGlobal.maxFSMs.containsKey(c_parent)) this.maxFSMtoRemove.add(c_parent);
        }
    }
    ...
    private void gSpanDFSInc(DFSCode c, Set<Integer> graphIdsInc, Map<DFSCode, Set<Integer>> SubgraphsToExtend, List<DFSCode> InfreToRemove, List<DFSCode> CanMaxToRemove) throws Exception {
        // If we have reached the maximum size, we do not need to extend this graph
        if (c.size() == Settings.maxNumberOfEdges - 1) {
            return;
        }
...
//        int number = 0;
        for (Entry<ExtendedEdge, Set<Integer>> entry : extensions.entrySet()) {
            ...
            if (this.frequentSubgraphs.containsKey(newC)) {
                if (!c.isEmpty()) {// have been generated, so we need to update it's support id
                    this.frequentSubgraphs.get(c).addAll(graphIdsInc);
                }
                gSpanDFSInc(newC, newGraphIDs, SubgraphsToExtend, InfreToRemove, CanMaxToRemove);
                continue;
            }
            //更新边界1：新边界中的新祖先
            if (c.isEmpty() && !DataGlobal.InfrequentSubgraphs.containsKey(newC)) {//new ancestor
                int v1L = extension.getvLabel1();
                int v2L = extension.getvLabel2();
                int eL = extension.getEdgeLabel();
                String edge;
                if (v1L <= v2L) {
                    edge = v1L + "_" + eL + "_" + v2L;
                } else {
                ...
                newGraphIDs.addAll(this.FrequentEdge.get(edge)[1]);
                SubgraphsToExtend.put(newC, newGraphIDs);
                Child_Parent.put(newC, new DFSCode());
                continue;
            }
            //更新边界2：三种边界处理
            if (this.frequentSubgraphs.containsKey(c) && DataGlobal.InfrequentSubgraphs.containsKey(newC)) {//old leaves,之后判断是否频繁，省略计算的部分
                //更新支持度
                this.frequentSubgraphs.get(c).addAll(graphIdsInc);

                DataGlobal.InfrequentSubgraphs.get(newC).addAll(newGraphIDs);
                newGraphIDs = DataGlobal.InfrequentSubgraphs.get(newC);

                if (newGraphIDs.size() >= Settings.minSup) {//若频繁则去掉（由不频繁变为频繁），否则就保留，第1，2种边界的处理
                    SubgraphsToExtend.put(newC, newGraphIDs);
                    this.Child_Parent.put(newC, c);
                    if (DataGlobal.maxFSMs.containsKey(c)) this.maxFSMtoRemove.add(c);
                    InfreToRemove.add(newC);
                }
                continue;
            }
            if (this.frequentSubgraphs.containsKey(c)) {
                if (isCanonical(newC)) {
                    if (!DataGlobal.InfrequentSubgraphs.containsKey(newC)) {//第三种边界的处理，new leaves
                        //更新支持度
                        this.frequentSubgraphs.get(c).addAll(graphIdsInc);
                        newGraphIDs = this.frequentSubgraphs.get(c);
                        if (newGraphIDs.size() >= Settings.minSup) {
                            SubgraphsToExtend.put(newC, newGraphIDs);
                            this.Child_Parent.put(newC, c);
                            CanMaxToRemove.add(newC);
                        }
                        continue;
                    }
                }
...
//        if (!c.isEmpty() && FreDesCount == 0) {
//            this.PseudoMaxSub.add(c.copy());
//        }
        // check the memory usage
        MemoryLogger.getInstance().checkMemory();
